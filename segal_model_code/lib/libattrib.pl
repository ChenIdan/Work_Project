...
require "/home/chen/nucleosome_model/lib/libfile.pl";
require "/home/chen/nucleosome_model/lib/liblist.pl";
require "/home/chen/nucleosome_model/lib/libset.pl";

##------------------------------------------------------------------------
#  \%translated attribTranslate ( \%target, \%attrib translator, $int only_translated=1 )
#
#  Treats all of the values in the target as keys in the translator
#  hash.  It then sets the new value equal to the value in the
#  translator hash.  For example if there exists $$target{'x'} = 'y'
#  and $$translator{'y'} = 'z' then the new hash %translated will have
#  $translated{'x'} = 'z'.
#
#  If only_translated==1 then %translated will only contain entries
#  that were successfully translated.  If only_translated==0 then it contains
#  all of the attributes in \%target even those that were not translated.
##------------------------------------------------------------------------
sub attribTranslate
{
   my ($target, $translator, $only_translated) = @_;

   $only_translated = defined($only_translated) ? $only_translated : 1;

   my %translated;

   foreach my $attrib (keys(%{$target}))
   {
      my $value = $$target{$attrib};

      if(exists($$translator{$value}))
      {
         $translated{$attrib} = $$translator{$value};
      }
      elsif(not($only_translated))
      {
         $translated{$attrib} = $value;
      }
   }
...
##------------------------------------------------------------------------
#  \%translated = attribTranslateFile(\%target, $file, $int only_translated=1,
#                                     $string delim="\t",
#                                     $int attrib_col=0, $int value_col=1)
#
#  Same as &attribTranslate() except it reads the translator
#  mapping from a file.
#
#  This allows very large hashes to be used efficiently to map
#  from the target space to a new space.
##------------------------------------------------------------------------
sub attribTranslateFile
{
   my ($target, $file, $only_translated, $delim, $attrib_col, $value_col) = @_;

   my @values = values(%{$target});

   my $values = &list2Set(\@values);

   my $translator = &attribRead($file, $delim, $attrib_col, $value_col,
                                0, 0, $values, 1);

   return &attribTranslate($target, $translator, $only_translated);
}
...
##------------------------------------------------------------------------
## Sets all of the values in a hash to undef.
##------------------------------------------------------------------------
sub attribSetUndef
{
   my ($attribs) = @_;
...
##                      \%attrib aliases=undef, $int restrict=0)
##
## multi - store multiple values for each attribute? If 1 stores sets for
##         each, if 2 stores lists for each.
##
## restrict - if 1: instead of mapping the keys using the aliases, don't remap,
##            just check for existence in aliases but keep the original
##            attribute name.
##------------------------------------------------------------------------
sub attribRead
{
   my ($file, $delim, $attrib_col, $value_col,
       $bidirectional, $multi, $aliases, $restrict) = @_;

   my $filep = ((-f $file) or (-l $file)) ? &openFile($file) : $file;

   my $attribs = &attribReadFp($filep, $delim, $attrib_col, $value_col,
                               $bidirectional, $multi, $aliases, $restrict);
...
sub attribReadFp
{
   my ($filep, $delim, $attrib_col, $value_col,
       $bidirectional, $multi, $aliases, $restrict) = @_;
   $delim         = not(defined($delim))         ? "\t" : $delim;
   $attrib_col    = not(defined($attrib_col))    ? 0    : $attrib_col;
   $value_col     = not(defined($value_col))     ? 1    : $value_col;
   $bidirectional = not(defined($bidirectional)) ? 0    : $bidirectional;
   $multi         = not(defined($multi))         ? 0    : $multi;
   $restrict      = not(defined($restrict))      ? 0    : $restrict;
   ...
   while(<$filep>)
   {
      ...
      my $attribute = $tuple[$attrib_col];

      my $value     = $tuple[$value_col];

      if(defined($aliases))
      {
         if(exists($$aliases{$attribute}))
         {
            $attribute = $restrict ? $attribute : $$aliases{$attribute};
         }
         else
         {
            $attribute = undef;
...
##---------------------------------------------------------------------------##
## \%attrib attribReadTuplesFp (\*FILE file, $int key_col=0, $string delim="\t")
##---------------------------------------------------------------------------##
sub attribReadTuplesFp
{
   my ($filep, $key_col, $delim) = @_;
...
##---------------------------------------------------------------------------##
## \%attrib attribReadTuples ($string filename, $int key_col=0)
##---------------------------------------------------------------------------##
sub attribReadTuples
{
   my @args = @_;
   ...
   if(open($filep, $filename))
   {
      $attrib = &attribReadTuplesFp($filep, @args);
   }
