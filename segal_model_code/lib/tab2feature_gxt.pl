...
while(<$file_ref>)
{
  ...
  if (length($feature_types2id{$row[4]}) == 0)
  {
    $feature_types2id{$row[4]} = $feature_counter++;
    push(@feature_types, $row[4]);
  }
...
for (my $i = 0; $i < @features; $i++)
{
    ...
    if ($add_feature_name_to_id == 1) { $row[1] .= " ($feature_types2id{$row[4]})"; }
    if ($add_counter_to_id == 1) { $row[1] .= " (ID$counter)"; $counter++;}

    print "$row[0]\t$row[1]\t$row[2]\t$row[3]\t$feature_types2id{$row[4]}\t$row[5]\n";
}
...
/home/chen/nucleosome_model/lib/tab2feature_gxt.pl <file> 
    ...
    -l <str>:           Location Display mode (Color gradient/Filled box/Unfilled box/Filled oval/Unfilled oval) (default: Color gradient)
