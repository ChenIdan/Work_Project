...
require "/home/chen/nucleosome_model/lib/format_number.pl";
require "/home/chen/nucleosome_model/lib/load_args.pl";
...
my $prescan_file=0;
if ($bins ne "" or $rescale ne "" or $probdist ne ""){ $prescan_file=1 }
...
if ($prescan_file){
  ...
  while (my $r=<$file_ref>){
	...
	if ($row[$i]>0){$col_sum_array[$i]+=$row[$i]}
...
while (my $line=<$file_ref>)
{
  ...
  for (my $i = $skip_columns; $i < scalar(@row); $i++)
  {
    if (($columns_hash{$i} eq "1" or $apply_to_all_columns == 1) and &pass_filter($row[0], $headers[$i], $row[$i]))
    {
      ...
      if ($substr != 0 and length($num) > $substr)
      {
...
###
	...
	$num *= $multiply_number;
	$num /= $divide_number ;
	if ($take_log == 1) { $num = $num > 0 ? log($num) : 0; }
	if ($take_log2 == 1) { $num = $num > 0 ? log($num) / $LOG2 : 0; }
	if ($take_log10 == 1) { $num = $num > 0 ? log($num) / $LOG10 : 0; }
	...
	if (length($precision) > 0) { $num = &format_number($num, $precision); }
...
/home/chen/nucleosome_model/lib/modify_column.pl <file>
